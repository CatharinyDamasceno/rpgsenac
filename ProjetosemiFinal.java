...
//Switch case Da Opção
    ...
    static void Instructions() {
        ...
        System.out.println("O jogo se baseia em uma luta entre Batman e seu inimigo Charada e seus capangas, dentro de seu covil maléfico \n"
                + "onde em meio a essa luta frenética, podem ser trocados socos, ataque de equipamentos ou até mesmo responder uma charada para sair com vida de lá! \n "
                + "Será apresentado as opções de ataques e de resposta e o usário deverá escolher-lá utilizando "
                + "o numero da opção desejada de forma correspondente \n"
        );
    ...
    static int Riddles(int[] vet, int controlQuest) {
        ...
        if (res != 9) { //Se a pessoa digitar 9 as questões são puladas 
            ...
            if (QuestionList[vet[cont]][opc[res]].equals(QuestionList[vet[cont]][5])) {
                System.out.println("\nAcertou");
                System.out.println("Resposta certa: " + QuestionList[vet[cont]][opc[res]] + "\n");
                ac++; // Caso o Usuario acerte a questão, o acerto volta como 1
                // Assim acrecentando na pontuação

            } else {
                System.err.println("Errou");
                System.out.println("Resposta Certa: " + QuestionList[vet[cont]][5] + "\n");
    ...
    static void Battle() { // Função do jogo
        ...
        while (hpUsuario > 0 && escolhaAtaque != 9 && controlQuest < 20) {
            ...
            if (hpUsuario > 0 && hpComputador > 0 && controlQuest < 20) {
                if (escolhaAtaque != 9) {
        ...
        if (escolhaAtaque != 9 && hpUsuario > 0) {  // Mensagem de saida caso O jogador vença 
            ...
            System.out.println("Sinto Muito Você foi derrotado \nSaude chegou a 0 \n");
        }
    }
